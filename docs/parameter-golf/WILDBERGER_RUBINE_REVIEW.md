# Review: Wildberger & Rubine in light of Q²
...
The first pass assessed this paper narrowly against Q²'s embedding retrieval
pipeline and dismissed most of the machinery. That was wrong — it evaluated
against the *application* rather than the *framework*. With Q² positioned as a
...
**Previously dismissed** because Q²'s embedding thresholds come from empirical
...
where $C_\mathbf{m}$ is the hyper-Catalan number. For Q² generalized, this
means:
...
from a reservoir sample using empirical percentiles. A general Q² framework
...
**The connection.** Q²'s run-reduced transition sequences form a trie where:
...
- **Error-correcting codes** over $\mathbb{Z}_4$ (the Gray map already connects
  Q² to the Nordstrom-Robinson and Kerdock codes via the Hammons et al. 1994
...
**What this gives Q².** Exact formulas for:
...
Q²-general, this becomes a statement about **hierarchical quantization**:
...
possibilities. For Q²'s transition key:

| Level | Paper | Q² (specific) | Q² (general) |
|:-----:|:------|:--------------|:-------------|
...
**Concrete consequence.** A general Q² framework should support multi-resolution
...
**Concrete consequence.** When extending Q² to higher-order alphabets ($q = 8$,
...
**Previously dismissed** because Q² retrieval doesn't need to invert. But
...
two faces of the same coin. For Q² generalized:
...
**Concrete consequence.** If Q² ever needs a decode path (e.g., for lossy
...
That's it. Everything else has a plausible Q²-general interpretation.
...
| Paper section | First pass | Revised (general Q²) |
|:--------------|:-----------|:---------------------|
...
into a near-complete algebraic companion to the Q² framework.
